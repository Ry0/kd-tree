...
int main() {
  // check that it'll find nodes exactly MAX away
  {
    ...
    exact_dist.insert(triplet(6.999, 4, 0));
    exact_dist.insert(triplet(7, 3.99, 0));
    exact_dist.insert(triplet(5, 4, 0));
    exact_dist.insert(triplet(1, 0, 7));
    ...
    std::cout << "Test find_nearest(), found at exact distance away from "
              << target << ", found " << *found.first << std::endl;
  }

  // do the same test, except use alternate_triplet as the search key
  // {
  //   // NOTE: stores triplet, but we search with alternate_triplet
  //   typedef KDTree::KDTree<2, triplet, alternate_tac> alt_tree;

  //   triplet actual_target(7, 0, 0);

  //   alt_tree tree;
  //   tree.insert(triplet(1, 0, 7));
  //   tree.insert(triplet(0, 1, 7));
  //   tree.insert(triplet(0, 2, 7));
  ...
  //   std::pair<alt_tree::const_iterator, double> found =
  //       tree.find_nearest(target);
  //   assert(found.first != tree.end());
  //   std::cout << "Test with alternate search type, found: " << *found.first
  //             << ", wanted " << actual_target << std::endl;
  ...
  //   // call find_nearest without a range value - it found a compile error
  //   // earlier.
  //   std::pair<tree_type::const_iterator, double> found =
  //       exact_dist.find_nearest(target);
  //   assert(found.first != exact_dist.end());
  //   std::cout << "Test find_nearest(), found at exact distance away from "
  //             << target << ", found " << *found.first << " @ " << found.second
  //             << " should be " << std::sqrt(8) << std::endl;
  ...
  //   std::pair<tree_type::const_iterator, double> found =
  //       exact_dist.find_nearest(target, std::sqrt(8));
  //   assert(found.first != exact_dist.end());
  //   std::cout << "Test find_nearest(), found at exact distance away from "
  //             << target << ", found " << *found.first << " @ " << found.second
  //             << " should be " << std::sqrt(8) << std::endl;
  ...
  //   int i = 0;
  //   for (tree_type::const_iterator iter = t.begin(); iter != t.end();
  //        ++iter, ++i)
  //     ;
  //   std::cout << "iterator walked through " << i << " nodes in total"
  //             << std::endl;
  //   if (i != 6) {
  //     std::cerr << "Error: does not tally with the expected number of nodes (6)"
  //               << std::endl;
  //     return 1;
  //   }
  //   i = 0;
  //   for (tree_type::const_reverse_iterator iter = t.rbegin(); iter != t.rend();
  //        ++iter, ++i)
  //     ;
  //   std::cout << "reverse_iterator walked through " << i << " nodes in total"
  //             << std::endl;
  //   if (i != 6) {
  //     std::cerr << "Error: does not tally with the expected number of nodes (6)"
  //               << std::endl;
  ...
  //   std::cout << "counted " << count << " nodes within range " << RANGE
  //             << " of " << s << ".\n";
  ...
  //   for (tree_type::const_iterator target = t.begin(); target != t.end();
  //        ++target) {
  //     std::pair<tree_type::const_iterator, double> found =
  //         t.find_nearest(*target, 0);
  ...
  //     std::pair<tree_type::const_iterator, double> notfound =
  //         t.find_nearest(s, small_dist);
  //     std::cout << "Test find_nearest(), nearest to " << s << " within "
  //               << small_dist << " should not be found" << std::endl;

  //     if (notfound.first != t.end()) {
  //       std::cout << "ERROR found a node at dist " << notfound.second << " : "
  //                 << *notfound.first << std::endl;
  //       std::cout << "Actual distance = " << s.distance_to(*notfound.first)
  //                 << std::endl;
  //     }
  ...
  //   {
  //     std::pair<tree_type::const_iterator, double> nif =
  //         t.find_nearest_if(s, std::numeric_limits<double>::max(), Predicate());
  //     std::cout << "Test find_nearest_if(), nearest to " << s << " @ "
  //               << nif.second << ": " << *nif.first << std::endl;

  //     std::pair<tree_type::const_iterator, double> cantfind = t.find_nearest_if(
  //         s, std::numeric_limits<double>::max(), FalsePredicate());
  //     std::cout << "Test find_nearest_if(), nearest to " << s
  //               << " should never be found (predicate too strong)" << std::endl;
  //     assert(cantfind.first == t.end());
  //   }

  //   {
  //     std::pair<tree_type::const_iterator, double> found =
  //         t.find_nearest(s, std::numeric_limits<double>::max());
  //     std::cout << "Nearest to " << s << " @ " << found.second << " "
  //               << *found.first << std::endl;
  //     std::cout << "Should be " << found.first->distance_to(s) << std::endl;
  //     // NOTE: the assert does not check for an exact match, as it is not exact
  //     // when -O2 or -O3 is
  //     // switched on.  Some sort of optimisation makes the math inexact.
  //     assert(fabs(found.second - found.first->distance_to(s)) <
  ...
  //     triplet s2(10, 10, 2);
  //     std::pair<tree_type::const_iterator, double> found =
  //         t.find_nearest(s2, std::numeric_limits<double>::max());
  //     std::cout << "Nearest to " << s2 << " @ " << found.second << " "
  //               << *found.first << std::endl;
  //     std::cout << "Should be " << found.first->distance_to(s2) << std::endl;
  //     // NOTE: the assert does not check for an exact match, as it is not exact
  //     // when -O2 or -O3 is
  //     // switched on.  Some sort of optimisation makes the math inexact.
  //     assert(fabs(found.second - found.first->distance_to(s2)) <
  //            std::numeric_limits<double>::epsilon());
  //   }
  ...
  // // Walter reported that the find_within_range() wasn't giving results that
  // // were within
  ...
  //             << vectors.size() << " candidates." << std::endl;

  //   // double-check the ranges
  //   for (std::deque<triplet>::iterator v = vectors.begin(); v != vectors.end();
  //        ++v) {
  //     double dist = sv.distance_to(*v);
  //     std::cout << "  " << *v << " dist=" << dist << std::endl;
  //     if (dist > 10.0f)
  //       std::cout << "    This point is too far! But that is by design, its "
  //                    "within a 'box' with a 'radius' of 10, not a sphere with "
  //                    "a radius of 10"
  //                 << std::endl;
  //     // Not a valid test, it can be greater than 10 if the point is in the
  //     // corners of the box.
  //     // assert(dist <= 10.0f);
  //   }
  // }

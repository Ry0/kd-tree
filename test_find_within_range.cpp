...
struct kdtreeNode {
  ...
  value_type operator[](size_t n) const { return xyz[n]; }
...
int main(int argc, char *argv[]) {
  ...
  for (size_t n = 0; n < 5; ++n) {
    ...
    cout << "data(x,y,z)=(" << node.xyz[0] << "," << node.xyz[1] << "," << node.xyz[2] << ")" << endl;
    node.index = n;
  ...
  cout << "sample(x,y,z)=(" << refNode.xyz[0] << "," << refNode.xyz[1] << "," << refNode.xyz[2] << ")" << endl;
  ...
  tree.find_within_range(refNode, limit,
                         back_insert_iterator< vector<kdtreeNode> >(howClose));

  // make sure no extra points are returned, and the return has no missing
  // points.
